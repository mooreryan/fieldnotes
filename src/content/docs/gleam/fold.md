...
Anyway, the version of `fold` that we specified with concrete types satisfies the original type signature that uses type variables. However, this type signature would not, since it doesn't follow the rules specified by `fold`'s type signature.
...
- The [tutorial](/gleam/event_driven_xml_parsing) on event-driven XML parsing in Gleam, which uses a `fold_signals` function similar to this one.
- The HaskellWiki entry for [Fold](https://wiki.haskell.org/index.php?title=Fold)

TODO: these internal links will break if the slug changes...
